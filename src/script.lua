...
function gpu_temp()
    ...
    gpu=tonumber(conky_parse('${nvidia temp}'))

    if gpu>warn_gpu then
        settings_table[5]['fg_colour']=warn
    else
        if gpu>crit_gpu then
...
function iface_watch()
    iface=conky_parse("${if_existing /proc/net/route enp2s0}enp2s0${else}wlp3s0${endif}")
